...
class RepartidorV {
    ...
    private var repartidorEnEdicion by mutableStateOf<RepartidorM?>(null)
    private val mensaje = MutableStateFlow<String?>(null)
    private var mostrarDialogoConfirmacion by mutableStateOf(false)
    private var repartidorAEliminar by mutableStateOf<RepartidorM?>(null)
    ...
    fun setVisible(drawerState: DrawerState, scope: CoroutineScope) {
        ...
        if (mostrarDialogoConfirmacion) {
            AlertDialog(
                onDismissRequest = {
                    ...
                    repartidorAEliminar = null
                },
                title = { Text("Confirmar Eliminación") },
                text = {
                    Text("¿Está seguro de que desea eliminar al repartidor \"${repartidorAEliminar?.nombre}\"? Esta acción no se puede deshacer.")
                },
                confirmButton = {
                    TextButton(
                        onClick = {
                            repartidorAEliminar?.id?.let { id ->
                                eliminarListener(id)
                                if (repartidorEnEdicion?.id == id) {
                                    repartidorEnEdicion = null
                                }
                            }
                            mostrarDialogoConfirmacion = false
                            repartidorAEliminar = null
        ...
        Scaffold (
            ...
            Column(
                ...
                Lista(onEditarClick = { repartidor ->
                    repartidorEnEdicion = repartidor
    ...
    private fun Formulario() {
        ...
        LaunchedEffect(repartidorEnEdicion) {
            repartidorEnEdicion?.let { repartidor ->
                nombre = repartidor.nombre ?: "Sin nombre"
                celular = repartidor.celular.toString()
                direccion = repartidor.direccion.toString()
            } ?: run {
                nombre = ""
                celular = ""
                direccion = ""
        ...
        Row(
            ...
            Button(
                onClick = {
                    if (repartidorEnEdicion != null && repartidorEnEdicion?.id != null) {
                        editarListener(repartidorEnEdicion!!.id!!, nombre, celular, direccion)
                        repartidorEnEdicion = null
                    } else {
                ...
                Text(if (repartidorEnEdicion != null) "Actualizar" else "Agregar")
            }

            if (repartidorEnEdicion != null) {
                Button(
                    onClick = {
                        repartidorEnEdicion =  null
                        nombre = ""
    ...
    @Composable
    private fun Lista(onEditarClick: (RepartidorM) -> Unit) {
        val repartidores by repartidores.collectAsState(initial = emptyList())
        ...
        if (repartidores.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(repartidores.size) { index ->
                    RepartidorItem(
                        repartidor = repartidores[index],
                        onEditar = { onEditarClick(repartidores[index]) },
                        onEliminar = {
                            repartidorAEliminar = repartidores[index]
    ...
    private fun RepartidorItem(
        ...
        Card(
            ...
            Column(
                ...
                Text (
                    text = repartidor.nombre ?: "sin nombre",
                ...
                Text (
                    text = "Dirección: ${repartidor.direccion}",
                    style = MaterialTheme.typography.bodyMedium
                )
                Spacer(modifier = Modifier.height(8.dp))
                Row(
